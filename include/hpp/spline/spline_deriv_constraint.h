...
struct spline_deriv_constraint : public exact_cubic<Time, Numeric, Dim, Safe, Point, T_Point, SplineBase> {
 ...
 public:
  ...
  spline_deriv_constraint(In wayPointsBegin, In wayPointsEnd,
  ...
  spline_deriv_constraint(const spline_deriv_constraint& other) : exact_cubic_t(other.subSplines_) {}
 ...
 private:
  ...
  t_spline_t computeWayPoints(In wayPointsBegin, In wayPointsEnd, const spline_constraints& constraints) const {
    ...
    ++next;
    for (std::size_t i(0); next != end; ++next, ++it, ++i) compute_one_spline<In>(it, next, cons, subSplines);
